                    ...
                    <div class="table-responsive rounded-4 bg-card shadow-sm mb-4">
                        <table class="table modern-table align-middle mb-0 table-hover custom-table">
                            <thead class="bg-light text-dark">
                                <tr>
                                    <th width="40%" class="ps-4 py-3 align-middle text-body small fw-bold text-uppercase">
                                        Sản phẩm / Phân loại</th>
                                    <th class="text-center py-3 align-middle text-body small fw-bold text-uppercase">Đã mua
                                    </th>
                                    <th class="text-center py-3 align-middle text-body small fw-bold text-uppercase">Hoàn
                                        trả</th>
                                    <th class="text-end pe-4 py-3 align-middle text-body small fw-bold text-uppercase">Hoàn
                                        tiền (Dự kiến)</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($return->items as $rItem)
                                    @php
                                        ...
                                        $variantLabel = '';
                                        if ($variant) {
                                            // Ưu tiên tên từ attribute values (VD: "Đỏ / L")
                                            $attrLabels = $variant->attributeValues->pluck('value')->filter()->implode(' / ');
                                            $variantLabel = $attrLabels ?: $variant->name ?: $variant->sku;
                                        }
                                    @endphp
                                    <tr>
                                        <td class="ps-4">
                                            ...
                                            @if($variantLabel)
                                                <small class="text-muted">
                                                    <i class="fas fa-tag fa-xs me-1"></i>{{ $variantLabel }}
                                                </small>
                                            @endif
                                        </td>
                                ...
                                @foreach($return->images as $img)
                                    ...
                                    <a href="javascript:void(0)"
                                        ...
                                        <img src="{{ asset('storage/' . $img) }}" class="object-fit-cover"
                                            style="width: 120px; height: 120px;" alt="Proof Image">
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                            ...
                            @if($return->status == 'pending')
                                <span
                                    class="badge bg-light-warning text-body border border-warning border-opacity-10 py-2 px-3 rounded-pill fw-normal font-monospace">Chờ
                                    xử lý</span>
                            @elseif($return->status == 'approved')
                                <span
                                    class="badge bg-light-success border border-success border-opacity-10 py-2 px-3 rounded-pill fw-normal font-monospace">Đã
                                    duyệt</span>
                            @elseif($return->status == 'rejected')
                                <span
                                    class="badge bg-light-danger border border-danger border-opacity-10 py-2 px-3 rounded-pill fw-normal font-monospace">Đã
                                    từ chối</span>
                            @endif
                        </li>
                    </ul>
                    ...
                    @if($return->status != 'pending')
                        <div class="{{ $return->status == 'approved' ? 'alert-success' : 'alert-danger' }} mt-3 mb-0">
                            <small class="d-block text-muted mb-2">
                                <i class="bx {{ $return->status == 'approved' ? 'bx-check' : 'bx-x' }} me-2"></i> Xử lý lúc:
                    ...
                    <div class="card shadow mb-4 rounded-4 shadow-sm bg-card">
                        ...
                        <div class="card-body bg-card rounded-bottom-4">
                            <form action="{{ route('admin.returns.update', $return) }}" method="POST" id="processReturnForm">
                                ...
                                <div class="mb-3">
                                    <label class="form-label fw-bold small text-uppercase">Ghi chú xử lý <span
                                ...
                                <div class="d-grid gap-2">
                                    <button type="button" class="btn btn-success py-2 fw-bold" data-bs-toggle="modal"
                                        data-bs-target="#approveModal">
                                        <i class="fas fa-check-circle me-1"></i> Duyệt & Hoàn kho
                                    </button>

                                    <button type="button" class="btn btn-danger py-2 fw-bold" data-bs-toggle="modal"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                ...
                <div class="modal-footer border-0 justify-content-center pb-4">
                    ...
                    <button type="submit" form="processReturnForm" name="action" value="approve"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                ...
                <div class="modal-footer border-0 justify-content-center pb-4">
                    ...
                    <button type="submit" form="processReturnForm" name="action" value="reject"
