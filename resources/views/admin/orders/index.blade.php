    ...
    <div class="container-fluid px-0">
        ...
        <div class="card-header bg-card border-bottom p-3 mb-4 rounded-4 shadow-sm">
            ...
            <form method="GET" action="{{ route('admin.orders.index') }}"
                ...
                <div class="input-group bg-light rounded-pill px-3 py-1 border border-light focus-ring flex-grow-1 flex-md-grow-0"
                    style="min-width: 200px; max-width: 300px;">
                    <span class="input-group-text bg-transparent border-0 pe-2 text-body"><i
                ...
                <div class="d-flex flex-wrap align-items-center gap-2 justify-content-end flex-grow-1">
                    ...
                    @if(request()->anyFilled(['search', 'status', 'month', 'year']))
                        <a href="{{ route('admin.orders.index') }}"
                            ...
                            data-bs-toggle="tooltip" title="Xóa bộ lọc" style="width: 36px; height: 36px;">
                            <i class='bx bx-refresh fs-5'></i>
                        </a>
                    @endif

                    <div class="bg-light rounded-pill p-1 d-inline-flex align-items-center overflow-auto shadow-sm"
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['status' => null, 'page' => 1]) }}"
                            ...
                            data-bs-toggle="tooltip" title="Tất cả">
                            Tất cả
                        </a>
                        ...
                        <a href="{{ request()->fullUrlWithQuery(['status' => 'confirmed', 'page' => 1]) }}"
                            class="d-flex align-items-center justify-content-center btn btn-sm rounded-pill px-3 transition-all {{ request('status') == 'confirmed' ? 'bg-white shadow-sm text-body fw-bold' : 'text-muted border-0 hover-text-body' }}"
                            data-bs-toggle="tooltip" title="Đã xác nhận">
                            <i class="bx bx-stamp"></i>
                        </a>

                        <a href="{{ request()->fullUrlWithQuery(['status' => 'shipping', 'page' => 1]) }}"
                            class="d-flex align-items-center justify-content-center btn btn-sm rounded-pill px-3 transition-all {{ request('status') == 'shipping' ? 'bg-white shadow-sm text-body fw-bold' : 'text-muted border-0 hover-text-body' }}"
                            data-bs-toggle="tooltip" title="Đang giao hàng">
                            <i class="bx bx-truck"></i>
                        </a>

                        <a href="{{ request()->fullUrlWithQuery(['status' => 'completed', 'page' => 1]) }}"
                            class="d-flex align-items-center justify-content-center btn btn-sm rounded-pill px-3 transition-all {{ request('status') == 'completed' ? 'bg-white shadow-sm text-body fw-bold' : 'text-muted border-0 hover-text-body' }}"
                            data-bs-toggle="tooltip" title="Hoàn thành">
                            <i class="bx bx-check"></i>
                        </a>

                        <a href="{{ request()->fullUrlWithQuery(['status' => 'cancelled', 'page' => 1]) }}"
                            class="d-flex align-items-center justify-content-center btn btn-sm rounded-pill px-3 transition-all {{ request('status') == 'cancelled' ? 'bg-white shadow-sm text-body fw-bold' : 'text-muted border-0 hover-text-body' }}"
                            data-bs-toggle="tooltip" title="Đã hủy">
                            <i class="bx bx-x"></i>
                        </a>
                    </div>
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <select name="month"
                            class="form-select form-select-sm rounded-pill bg-card border-0 shadow-sm cursor-pointer text-body fw-medium py-2 px-3"
                            ...
                            <tr class="transition-all hover-bg-light">
                                <td class="ps-4">
                                    <a href="{{ route('admin.orders.show', $order->code) }}"
                                        class="fw-bold text-body text-decoration-none font-monospace">
                                        #{{ $order->code }}
                                    </a>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="d-flex flex-column">
                                            <span
                                                class="fw-semibold text-body small">{{ $order->user->name ?? 'Khách vãng lai' }}</span>
                                            @if($order->user)<span class="text-body" style="font-size: 10px;">Thành
                                            viên</span>@endif
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span
                                        class="fw-bold text-success">{{ number_format($order->total_amount, 0, ',', '.') }}₫</span>
                                </td>
                                <td>
                                    @php
                                        $st = [
                                    ...
                                    @endphp
                                    <span
                                        class="badge {{ $st['bg'] }} {{ $st['text'] }} rounded-pill px-3 py-2 border border-{{ $st['color'] }} border-opacity-10 d-inline-flex align-items-center gap-1 fw-normal font-monospace">
                                        {{ $st['label'] }}
                                    </span>
                                </td>
                                <td>
                                    <div class="text-center">
                                        <span
                                            class="badge bg-light-{{ $order->payment_method == 'cod' ? 'secondary' : 'primary' }} rounded-pill px-3 py-2 text-body border border-{{ $order->payment_method == 'cod' ? 'secondary' : 'primary' }} border-opacity-10 fw-normal font-monospace">
                                ...
                                <td>
                                    <div class="d-flex flex-column">
                                        ...
                                        <span class="text-body"
                                            style="font-size: 10px;">{{ $order->created_at->format('H:i') }}</span>
                                    </div>
                                </td>
                                <td class="pe-4 text-center">
                                    <a href="{{ route('admin.orders.show', $order->code) }}"
                                        class="btn btn-icon btn-light-primary rounded-circle shadow-sm" data-bs-toggle="tooltip"
                                        title="Xem chi tiết">
