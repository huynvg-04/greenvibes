    ...
    <div class="container-fluid px-0">
                ...
                <div class="card border-0 shadow-sm rounded-4 h-100 card-hover">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-3">
                            ...
                            <div>
                                <p class="text-body text-uppercase fw-semibold small mb-1">Doanh thu</p>
                                <h4 class="fw-bold text-body mb-0">
                                    {{ number_format($revenueStats['this_month'] ?? 0, 0, ',', '.') }}<small
                                        class="small ms-1 text-body">₫</small></h4>
                            </div>
                        </div>
                        @php $growth = $revenueStats['growth'] ?? 0;
                        $isPositive = $growth >= 0; @endphp
                        <div class="d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="top"
                            ...
                            <span
                                class="badge {{ $isPositive ? 'bg-light-success text-success' : 'bg-light-danger text-danger' }} rounded-pill px-2 py-1 me-2 border border-0">
                                <i class='bx {{ $isPositive ? 'bx-trending-up' : 'bx-trending-down' }}'></i>
                                {{ $isPositive ? '+' : '' }}{{ $growth }}%
                            </span>
                ...
                <div class="card border-0 shadow-sm rounded-4 h-100 card-hover">
                    <div class="card-body p-4">
                        <div class="d-flex align-items-center mb-3">
                            ...
                            <div>
                                <p class="text-body text-uppercase fw-semibold small mb-1">Đơn hàng tháng</p>
                                <h4 class="fw-bold text-body mb-0">{{ number_format($orderStats['this_month'] ?? 0) }}</h4>
                            </div>
                        </div>
                        @php $growth = $orderStats['growth'] ?? 0;
                        $isPositive = $growth >= 0; @endphp
                        <div class="d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="top"
            ...
            <div class="col-12 col-md-6 col-lg-3">
                <div class="card border-0 shadow-sm rounded-4 h-100 card-hover">
                    <div class="card-body p-4">
                        ...
                        @php $growth = $productStats['growth'] ?? 0;
                        $isPositive = $growth >= 0; @endphp
                        <div class="d-flex align-items-center" data-bs-toggle="tooltip" data-bs-placement="top"
        ...
        <div class="row g-4">
            <div class="col-xl-8 col-lg-7">
                ...
                <div
                    class="card-header bg-white border-bottom p-4 d-flex justify-content-between align-items-center mb-4 shadow-sm rounded-4">
                    <h5 class="fw-bold text-body mb-0"><i class='bx bxs-time-five me-2 text-body'></i>Đơn hàng gần đây</h5>
                    @can('viewAny', App\Models\Order::class)
                        <a href="{{ route('admin.orders.index') }}"
                            class="btn btn-sm btn-light text-body fw-medium rounded-pill px-3">
                            Xem tất cả
                        </a>
                    @endcan
                </div>
                                ...
                                @forelse($recentOrders as $order)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <span
                                                class="badge bg-light-{{ $statusData['class'] }} text-{{ $statusData['class'] }} fw-normal rounded-pill px-3 py-2 border border-{{ $statusData['color'] }} border-opacity-10 me-2 font-monospace">
                                                {{ $statusData['label'] }}
                                            </span>
                                        </td>
            ...
            <div class="col-xl-4 col-lg-5">
                <div class="card-header bg-white border-bottom p-4 rounded-4 mb-4 shadow-sm">
                    <h5 class="fw-bold text-body mb-0 d-flex align-items-center"><i
                            class='bx bxs-trophy me-2 text-body'></i>Top 5 sản phẩm bán chạy</h5>
                </div>
                <div class="card-body p-0">
                    <div class="list-group list-group-flush rounded-4 shadow-sm bg-card">
                        @forelse($topProducts as $index => $product)
                            <div
                                class="list-group-item border-0 py-3 px-4 d-flex align-items-center hover-bg-light transition-all">
                                <div class="me-3">
                                    @if($index == 0)
                                        <div class="bg-warning text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm"
                                            style="width: 24px; height: 24px; font-size: 12px; font-weight: bold;">1</div>
                                    @elseif($index == 1)
                                        <div class="text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm"
                                            style="width: 24px; height: 24px; font-size: 12px; background-color: #9E9E9E; font-weight: bold;">
                                            2</div>
                                    @elseif($index == 2)
                                        <div class="text-white rounded-circle d-flex align-items-center justify-content-center shadow-sm"
                                            style="width: 24px; height: 24px; font-size: 12px; background-color: #CD7F32; font-weight: bold;">
                                            3</div>
                                    @else
                                        <div class="bg-light text-body rounded-circle d-flex align-items-center justify-content-center border"
                                            style="width: 24px; height: 24px; font-size: 12px; font-weight: bold;">{{ $index + 1 }}
                                        </div>
                                    @endif
                                </div>
                                <div class="me-3">
                                    @if(isset($product->image_url) && $product->image_url)
                                        <img src="{{ $product->image_url }}" class="rounded-3 border" width="48" height="48"
                                ...
                                <div class="flex-grow-1 min-w-0">
                                    <h6 class="mb-0 fw-semibold text-body text-truncate" style="font-size: 14px;"
                                        title="{{ $product->name }}">
                                        {{ $product->name }}
                                    </h6>
                                    <small
                                        class="text-body d-block text-truncate">{{ $product->category->name ?? 'Chưa phân loại' }}</small>
                                </div>
